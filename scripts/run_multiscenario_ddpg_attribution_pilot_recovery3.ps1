...
$RunRoot = (
    "results\multiscenario_gcn_flat_ddpg_attribution_cuda_pilot_recovery3"
)
$FinalOutput = (
    "results\multiscenario_gcn_flat_ddpg_attribution_cuda_pilot_recovery3_eval"
)
$FrozenOutput = (
    "results\" +
    "multiscenario_gcn_flat_ddpg_attribution_cuda_pilot_recovery3_pretrain_eval"
)
$GateRoot = (
    "results\multiscenario_gcn_ddpg_attribution_recovery3_stability_gate"
)
...
$LogPath = Join-Path $TransferRoot "pilot_recovery3_$Timestamp.txt"
$ComparisonPath = Join-Path $TransferRoot (
    "checkpoint_attribution_recovery3_$Timestamp.json"
)
...
try {
    $TestPatterns = @(
        ...
        "test_recovery3_contract.py",
        "test_stress_residual_anchor_checkpoint.py",
    ...
    $Recovery2CrashRoot = (
        "results\" +
        "multiscenario_gcn_ddpg_attribution_recovery2_stability_gate\" +
        "gcn_residual_mdl2_network_ddpg_afd\seed0\checkpoints"
    )
    $Recovery2CrashState = Join-Path $Recovery2CrashRoot (
        "gcn_residual_mdl2_network_ddpg_afd_seed0_training_state.pt"
    )
    $Recovery2CrashActor = Join-Path $Recovery2CrashRoot (
        "gcn_residual_mdl2_network_ddpg_afd_seed0_episode10.pt"
    )
    $Recovery2CrashStateExpectedHash = (
        "daf90359a4018206f232ee68b8c533e0ea758e0981b32e436672e33f743f8c75"
    )
    $Recovery2CrashActorExpectedHash = (
        "88f045e9a334253d85d75a8cef679f4a552ec5aaf45b7a510abe5c9b18cbbda8"
    )
    foreach ($Evidence in @(
        [pscustomobject]@{
            Path = $Recovery2CrashState
            ExpectedHash = $Recovery2CrashStateExpectedHash
        },
        [pscustomobject]@{
            Path = $Recovery2CrashActor
            ExpectedHash = $Recovery2CrashActorExpectedHash
        }
    )) {
        if (-not (Test-Path $Evidence.Path)) {
            throw "Missing immutable Recovery 2B crash evidence: $($Evidence.Path)"
        }
        $ActualHash = (
            Get-FileHash -Algorithm SHA256 $Evidence.Path
        ).Hash.ToLowerInvariant()
        if ($ActualHash -ne $Evidence.ExpectedHash) {
            throw "Recovery 2B crash evidence SHA256 mismatch: $($Evidence.Path)"
        }
    }
    $AnchorStressCalls = 150000
    Invoke-CheckedPython `
        -Stage "Recovery 2B replay-backed residual anchor stress" `
        -Arguments @(
            "-m", "evaluation.stress_residual_anchor_checkpoint",
            "--training-state", $Recovery2CrashState,
            "--actor-checkpoint", $Recovery2CrashActor,
            "--anchor-policy", "mdl2",
            "--calls", [string]$AnchorStressCalls,
            "--minimum-replay-rows", "520"
        )
    foreach ($Evidence in @(
        [pscustomobject]@{
            Path = $Recovery2CrashState
        ...
        if ($ActualHash -ne $Evidence.ExpectedHash) {
            throw "Replay stress modified immutable evidence: $($Evidence.Path)"
        }
    }
...
$ProvenancePath = Join-Path $TransferRoot (
    "provenance_recovery3_$Timestamp.json"
)
...
$Provenance = [ordered]@{
    ...
    replay_anchor_stress_calls = $AnchorStressCalls
    replay_anchor_stress_training_state_sha256 = (
        $Recovery2CrashStateExpectedHash
    )
    replay_anchor_stress_actor_sha256 = $Recovery2CrashActorExpectedHash
    ...
    recovery_attempt = 3
    recovery_reason = (
        "Recovery 2B diagnostic gate exited with Windows 0xC0000005 in " +
        "the residual-anchor shortage sort after completing episode 10."
    )
    software_mitigation = (
        "Replace repeated native NumPy argsort on small shortage vectors " +
        "with deterministic Python ordering and validate finite inputs."
    )
    anchor_equal_shortage_tie_break = "facility_index_ascending"
...
$ZipPath = Join-Path $TransferRoot (
    "multiscenario_ddpg_attribution_pilot_recovery3_$Timestamp.zip"
)
$ArchiveInputs = @(
    $GateRoot,
    $RunRoot,
...
Write-Host ""
Write-Host "Multi-scenario DDPG attribution recovery3 completed."
