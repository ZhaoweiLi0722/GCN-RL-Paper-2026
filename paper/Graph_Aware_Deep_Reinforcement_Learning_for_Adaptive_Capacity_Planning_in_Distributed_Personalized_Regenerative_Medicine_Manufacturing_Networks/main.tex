...
Distributed make-to-order manufacturing networks require adaptive decision policies because demand, supply availability, and resource states evolve over time and across connected facilities. Personalized regenerative medicine (PRM), and autologous cell therapy in particular, is a demanding instance of this class: the product is \emph{perishable}, subject to a hard viability deadline; it is \emph{identity-bound}, tied one-to-one to a single patient and non-fungible across patients; and its demand is \emph{patient-condition-driven}, since a patient whose health deteriorates while waiting may become clinically ineligible before treatment can be delivered. We formulate distributed PRM capacity planning as a constrained graph Markov decision process that treats these three properties as first-class, and we develop a simulation testbed realizing it---with patient-condition dynamics, expiry-aware inventory, and resource sharing restricted to fungible reagents and bioreactor capacity under the autologous identity constraint---that is reusable as a benchmark. The decision framework couples a graph convolutional state encoder, which captures inter-facility dependencies and the topology along which shortages and slack propagate, with continuous-control actor--critic reinforcement learning that learns feasibility-projected capacity, reagent, and replenishment actions from interaction with the environment. Rather than committing to a single algorithm, we evaluate a family of graph-aware backbones---DDPG, TD3, SAC, and PPO---treating the classical GCN--DDPG pairing as an ablation, and we add a flat-state ablation to separate the value of the graph encoder from that of reinforcement learning itself. Because deep reinforcement learning results are sensitive to random seeds and implementation details, we adopt a multi-seed protocol with interquartile-mean performance and confidence intervals and, given the clinical stakes, report patient-service outcomes such as eligibility and waiting alongside operating cost. Under supply disruption, demand forecast error, and a patient-condition stress regime, we assess whether graph representation and stability-oriented backbones improve coordinated policies over flat-state control and strong look-ahead heuristics. The study contributes an intelligent, network-aware decision-support approach for distributed healthcare manufacturing and a generalizable modeling framework for time-critical, make-to-order engineering systems with graph-structured dependencies.
\end{abstract}
...
\begin{figure}[htbp]
    ...
    \caption{Illustration of a two-facility distributed manufacturing system for personalized regenerative medicine. Each facility holds four state components: a patient--specimen queue whose members carry a deteriorating survival index $S_j$ (eligible while $S_j \ge \epsilon$), reagent inventory $r_t^n$, bioreactor capacity $\mathbf{b}_t^n$, and perishable finished product. Condition-driven patient demand $d_t^n$ and disruption-prone reagent replenishment $p_t^n$ arrive under uncertainty. The two \emph{fungible} resources---reagents and bioreactor capacity---can be rebalanced across facilities through transshipment $e_t^n$ and capacity relocation $q_t^n$ (solid green links). Patient specimens and finished product are \emph{identity-bound}: they cannot be pooled or reassigned across facilities (crossed red link). This fungible-only sharing under an identity constraint is the operational asymmetry that distinguishes distributed PRM manufacturing.}
