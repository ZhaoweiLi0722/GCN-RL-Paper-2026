...
\midrule
DDPG~\cite{Lillicrap2016DDPG} & Deterministic & Off-policy & Baseline actor--critic for continuous control; value overestimation and training instability motivate treating GCN--DDPG as an ablation rather than the proposed method. \\
TD3~\cite{Fujimoto2018TD3} & Deterministic & Off-policy & Clipped double-$Q$ learning, delayed policy updates, and target smoothing to curb overestimation and stabilize training. \\
...
The method initiates with a dual initialization: the actor and critic networks of the backbone and the GCN for graph state processing. Given an observed state \( X_t \), represented as raw graph data reflecting the current status of the manufacturing network, the GCN is used as a preprocessing unit. It transforms this raw graph into a condensed representation \( X'_t \), which retains the critical structural information of the original graph.
