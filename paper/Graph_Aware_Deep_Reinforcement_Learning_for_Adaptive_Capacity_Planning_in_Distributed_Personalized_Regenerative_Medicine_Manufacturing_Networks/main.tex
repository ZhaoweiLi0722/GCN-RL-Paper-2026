...
routing-enabled teacher is shared across the learned architectures. After two
five-episode seed-0 smoke runs pass identity, CUDA, update, finite-loss,
checkpoint, parameter-gap, and nonzero-route gates, the locked pilot runs six
independent 100-episode jobs in the order GCN seeds 0--2 followed by flat seeds
0--2. Final and frozen-pretraining checkpoints are evaluated on four routing
scenarios with 100 paired CRN replications per training seed; lead-zero
specimen transit and one-epoch product return are prespecified sensitivities.
The primary attribution is GCN versus matched flat, GCN versus MDL-2, and final
versus frozen pretraining. No-routing is retained only for deterministic
mechanics regression or a separately approved supplementary ablation, not as a
default formal training arm. No numerical result from this routing campaign is
reported until its locked outputs pass the preregistered gates.
...
The numerical findings currently summarized below come from the earlier
no-routing configurations and should be interpreted as controller evidence,
not as evidence for or against specimen routing. The calibrated patient process places both ISO and MDL-2 inside a clinically
...
conservation, lifecycle, and CRN tests, but the locked routing-primary teacher,
smoke, pilot, and holdout stages have not yet produced results. Accordingly,
this draft makes no empirical claim about the value of routing and does not
reinterpret any legacy no-routing checkpoint as routing-enabled evidence.
...
The immediate empirical priority is the locked routing-primary campaign:
generate a fresh routing teacher, pass the two routing smoke runs, complete the
six three-seed routing pilot runs, and evaluate GCN, matched flat, MDL-2, and
frozen-pretraining controls under the same CRN protocol. No-routing training is
reserved for a separately justified supplementary ablation rather than the
default experiment. If online-learning attribution remains inconclusive, future
