"""Conservative TD3 fine-tuning for the full AFR-GCN residual policy."""
...
class ConservativeGCNResidualTD3Agent(GCNDDPGAgent):
    """TD3 backend that preserves the complete distilled GCN policy surface.

    The legacy ``GCNTD3Agent`` predates the temporal encoder, network-residual
    edge heads, correction gates, and endpoint projection used by the regional
    Stage-3 checkpoints. Inheriting from ``GCNDDPGAgent`` keeps that policy
    surface checkpoint-compatible while replacing the single-critic update
    with conservative TD3.
    """
    ...
    def update(self) -> dict[str, float]:
        ...
        with torch.no_grad():
            ...
            if self.policy_noise > 0.0:
                target_noise = (
                    torch.randn_like(next_network_actions) * self.policy_noise
    ...
    def _actor_objective(
        ...
        actor_q = self._twin_min_q(node_features, actor_actions)
        if (
        ...
        with torch.no_grad():
            if self.actor_advantage_baseline == "reference":
                ...
                baseline_q = (
                    self._twin_max_q(node_features, baseline_actions)
                    if self.reference_advantage_use_upper_confidence
                    else self._twin_min_q(node_features, baseline_actions)
                )
            else:
                baseline_actions = (
                    self._base_actions_from_states_tensor(states)
                )
    ...
    def _twin_min_q(self, node_features, actions):
        return torch.minimum(
            self.critic1(node_features, actions),
            self.critic2(node_features, actions),
        )

    def _twin_max_q(self, node_features, actions):
