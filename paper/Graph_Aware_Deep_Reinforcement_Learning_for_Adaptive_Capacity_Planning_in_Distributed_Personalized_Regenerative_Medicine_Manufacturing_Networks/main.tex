...
conservation, lifecycle, and CRN tests, and routing-enabled teacher and
development screens have been completed. The untouched-seed routing-primary
GCN/flat confirmation and final holdout remain pending. Accordingly, this
draft treats the current routing screens as development evidence, makes no
confirmatory empirical claim about the value of routing, and does not
...
The immediate empirical priority is to freeze the current routing-primary DDPG
candidate, complete matched GCN and flat confirmation on five untouched seeds,
and evaluate both final and frozen-pretraining controls against routing MDL-2
under the same CRN protocol. Training begins with 100 episodes and is extended
uniformly to 200 or, only when justified by the still-sealed development
learning curves, 300 episodes. No-routing training is reserved for a separately
justified supplementary ablation rather than the default experiment. If
online-learning attribution remains inconclusive after this independent
confirmation, the next candidate should change the actor--critic mechanism
(for example, a matched TD3 study) rather than repeatedly tuning DDPG on the
same development stream. Longer-term experiments should test unseen clinic
