...
def solve_iso(d,L,x,Dtr,Atr,paras,p,beta):
    ...
    q0 = q1
    q2 = q1+1
    test_res1 = test_iso(d, L, x, Dtr, Atr, [q1,q0], paras, p, beta)
    test_res2 = test_iso(d, L, x, Dtr, Atr, [q2,q0], paras, p, beta)
...
def solve_iso_syn(d,L,x,Dtr,Atr,paras,p,beta):
    cB, hB, pB = paras[3], paras[4], paras[5]
    rhoB = (pB-(1-beta)*cB)/(pB+hB)
    q1 = max([0,poisson.ppf(rhoB,L*d[0])-x[0]['b'][0]])
    q2 = q1+1
    ...
    ec11, _, ec12, _ = cost_iso(test_res1, paras, beta)
    ec21, _, ec22, _ = cost_iso(test_res2, paras, beta)
    ec1 = ec11+ec12
    ec2 = ec21+ec22
    delta = ec2-ec1
    if delta == 0:
...
# In[]: LB
def get_Zdict_lb(dd, pp, paras, beta):
    cR, hR, pR = paras[0], paras[1], paras[2]
    rhoR = (pR-(1-beta)*cR)/(pR+hR)
    Zdict = np.zeros(30)
    z = poisson.ppf(rhoR,dd)
    for k in range(30):
        wv = np.array([pp**i*beta**i for i in range(k+1)])
        wv[0] = 1
        funv = np.array([-pR+(pR+hR)*poisson.cdf(z,dd*(i+1)) for i in range(k+1)])
        phi = (1-beta)*cR+np.dot(wv,funv)
        while phi<0:
            z = z+1
            funv = np.array([-pR+(pR+hR)*poisson.cdf(z,dd*(i+1)) for i in range(k+1)])
...
def get_a_iso_lb(xx,Zdict,At,t,T):
    a = 0
    if At == 0:
        a = max([0,xx['s']-xx['r']+Zdict[min([29,T-t])]])
    return a

def sim_iso_lb(dd,L,T,xx,DDv,AAv,BB,Zdict):
    sim_res = {}
    ...
    while t<T:
        ...
        a = get_a_iso_lb(xx, Zdict, AAv[t], t, T)
        av[t] = a
        
        m = np.min([xx['s'],xx['b'][0],xx['r']])
        xx['s'] = xx['s']-m+DDv[t]
        xx['b'][0] = xx['b'][0]-m+qv[t]+xx['b'][1]
        for i in np.arange(1,L-1):
            xx['b'][i] = xx['b'][i+1]
            xx['b'][i] = xx['b'][i+1]
        xx['b'][L-1] = m
        ...
        bsv[t] = np.max([xx['s']-xx['b'][0],0])
        rsv[t] = np.max([xx['s']-xx['r'],0])
        
        bov[t] = np.max([-xx['s']+xx['b'][0],0])
        rov[t] = np.max([-xx['s']+xx['r'],0])
        
        t = t+1
    
    sim_res = {'B':Bv,'s':sv,'b0':b0v,'r':rv,'a':av,'q':qv,'bs':bsv,'rs':rsv,'bo':bov,'ro':rov}    
    return sim_res

def test_iso_lb(dd,L,xx,DD,AA,BB,paras,pp,beta):
    test_res = {}
    N = np.shape(DD)[0]
    T = np.shape(DD)[1]
    Bm = np.zeros((N,T))
    ...
    Zdict = get_Zdict_lb(dd, pp, paras, beta)
    for n in range(N):
        sim_res = sim_iso_lb(dd,L,T,xx,DD[n],AA[n],BB,Zdict)
        Bm[n], sm[n], b0m[n], rm[n], am[n], qm[n], bsm[n], rsm[n], bom[n], rom[n] = sim_res['B'], sim_res['s'], sim_res['b0'],sim_res['r'],sim_res['a'],sim_res['q'],sim_res['bs'],sim_res['rs'],sim_res['bo'],sim_res['ro'] 
    test_res = {'B':Bm,'s':sm,'b0':b0m,'r':rm,'a':am,'q':qm,'bs':bsm,'rs':rsm,'bo':bom,'ro':rom}
    return test_res

def cost_iso_lb(test_res,paras,beta):
    cR, hR, pR, cB, hB, pB = paras[0], paras[1], paras[2], paras[3], paras[4], paras[5]
    am, qm, bom, rom, bsm, rsm = test_res['a'], test_res['q'], test_res['bo'], test_res['ro'],test_res['bs'], test_res['rs']
    N = np.shape(am)[0]
    T = np.shape(am)[1]
    betaT =np.matrix([beta**t for t in range(T)])
    oneN = np.transpose(np.matrix(np.ones(N)))
...
def solve_iso_lb(d,L,x,Dtr,Atr,paras,p,beta):
    ...
    cB, hB, pB = paras[3], paras[4], paras[5]
    rhoB = (pB-(1-beta)*cB)/(pB+hB)
    B1 = max([0,poisson.ppf(rhoB,L*dd)-xx['b'][0]])
    B2 = B1+1
    test_res1 = test_iso_lb(dd,L,xx,DD,AA,B1,paras,pp,beta)
    test_res2 = test_iso_lb(dd,L,xx,DD,AA,B2,paras,pp,beta)
    ec1,_ = cost_iso_lb(test_res1,paras,beta)
    ec2,_ = cost_iso_lb(test_res2,paras,beta)
    delta = ec2 - ec1
    if delta == 0:
        BB = B1
    if delta < 0:
