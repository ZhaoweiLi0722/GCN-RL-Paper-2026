...
def sim_co(d,L,T,x,Dv,Av,B,method,kwargs):
    ...
    while t < T:
        ...
        if method == 'emdl1':
            act = get_act_emdl(d,x,[Av[0][t],Av[1][t]],t,T,kwargs['paras'],min([T-t-1,1]),kwargs['p'],kwargs['beta'],kwargs['Zdict'])
        if method == 'emdl2':
            act = get_act_emdl(d,x,[Av[0][t],Av[1][t]],t,T,kwargs['paras'],min([T-t-1,2]),kwargs['p'],kwargs['beta'],kwargs['Zdict'])
...
# In[]:
def cost_co(test_res,B,paras,beta):
    ...
    costm1 = cR*am1+hB*bom1+hR*rom1+pB*bsm1+pR*rsm1+KS*0.5*np.absolute(wm1)+KR*0.5*np.absolute(em1)+KB*0.5*np.absolute(qm1)
    costdm1 = np.multiply(betam,costm1)
    costdv1 = np.sum(costdm1,axis=1)+cB*B[0]
    ec1 = np.sum(costdv1)/N
    
    costm2 = cR*am2+hB*bom2+hR*rom2+pB*bsm2+pR*rsm2+KS*0.5*np.absolute(wm2)+KR*0.5*np.absolute(em2)+KB*0.5*np.absolute(qm2)
    costdm2 = np.multiply(betam,costm2)
    costdv2 = np.sum(costdm2,axis=1)+cB*B[1]
...
def solve_co(d,L,T,x,B,Dtr,Atr,paras,p,beta,bmax,method,Num):
    ...
    ec1, _, ec2, _ = cost_co(test_res,B, paras, beta)
    c0 = ec1+ec2
    costm[B[0],B[1]] = c0
    indm[B[0],B[1]] = 1
    ...
    while search:
        ...
        if B[0]>0:
            ...
            if indm[B1[0],B1[1]]==0:
                ...
                ec1, _, ec2, _ = cost_co(test_res1,B1, paras, beta)
        ...
        if B[0]<bmax:
            ...
            if indm[B2[0],B2[1]]==0:
                ...
                ec1, _, ec2, _ = cost_co(test_res2,B2, paras, beta)
        ...
        if B[1]>0:
            ...
            if indm[B3[0],B3[1]]==0:
                ...
                ec1, _, ec2, _ = cost_co(test_res3,B3, paras, beta)
        ...
        if B[1]<bmax:
            ...
            if indm[B4[0],B4[1]]==0:
                ...
                ec1, _, ec2, _ = cost_co(test_res4,B4, paras, beta)
...
def solve_co_syn(d,L,T,x,B,Dtr,Atr,paras,p,beta,bmax,method,Num):
    ...
    ec1, _, ec2, _ = cost_co(test_res,[Bs,Bs], paras, beta)
    c0 = ec1+ec2
    costv[Bs] = c0
    indv[Bs] = 1
    ...
    while search:
        ...
        if Bs>0:
            ...
            if indv[Bs1] == 0:
                ...
                ec1, _, ec2, _ = cost_co(test_res1,[Bs1,Bs1], paras, beta)
                costv[Bs1] = ec1+ec2
                indv[Bs1] = 1
            c1 = costv[Bs1]
        else:
        ...
        if Bs<bmax:
            ...
            if indv[Bs2] == 0:
                ...
                ec1, _, ec2, _ = cost_co(test_res2,[Bs2,Bs2], paras, beta)
                costv[Bs2] = ec1+ec2
                indv[Bs2] = 1
            c2 = costv[Bs2]
        else:
...
def get_act_myo(d,x,At,paras):
    ...
    d_max = int(max([poisson.ppf(0.99,d[0]),poisson.ppf(0.99,d[1])]))
    pdv1 = poisson.pmf(np.arange(d_max+1),d[0])
    pdv2 = poisson.pmf(np.arange(d_max+1),d[1])
    ...
    q = M_myo.addVars(2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='q')
    qq = M_myo.addVars(2,obj=KB/2,vtype=GRB.CONTINUOUS,name='qq')
    w = M_myo.addVars(2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='w')
    ww = M_myo.addVars(2,obj=KS/2,vtype=GRB.CONTINUOUS,name='ww')
    e = M_myo.addVars(2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='e')
    ee = M_myo.addVars(2,obj=KR/2,vtype=GRB.CONTINUOUS,name='ee')
    Ord = M_myo.addVars(2,d_max+1,obj=hR*np.array([pdv1,pdv2]),lb=0,vtype=GRB.CONTINUOUS,name='Ord')
    Srd = M_myo.addVars(2,d_max+1,obj=pR*np.array([pdv1,pdv2]),lb=0,vtype=GRB.CONTINUOUS,name='Srd')
    Obd = M_myo.addVars(2,d_max+1,obj=hB*np.array([pdv1,pdv2]),lb=0,vtype=GRB.CONTINUOUS,name='Obd')
    Sbd = M_myo.addVars(2,d_max+1,obj=pB*np.array([pdv1,pdv2]),lb=0,vtype=GRB.CONTINUOUS,name='Sbd')
...
def get_act_emyo(d,x,At,t,T,paras,Zdict):
    ...
    q = M_emyo.addVars(2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='q')
    qq = M_emyo.addVars(2,obj=KB/2,vtype=GRB.CONTINUOUS,name='qq')
    w = M_emyo.addVars(2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='w')
    ww = M_emyo.addVars(2,obj=KS/2,vtype=GRB.CONTINUOUS,name='ww')
    e = M_emyo.addVars(2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='e')
    ee = M_emyo.addVars(2,obj=KR/2,vtype=GRB.CONTINUOUS,name='ee')
    Ord = M_emyo.addVars(2,d_max+1,obj=hR*np.array([pdv1,pdv2]),lb=0,vtype=GRB.CONTINUOUS,name='Ord')
    Srd = M_emyo.addVars(2,d_max+1,obj=pR*np.array([pdv1,pdv2]),lb=0,vtype=GRB.CONTINUOUS,name='Srd')
    Obd = M_emyo.addVars(2,d_max+1,obj=hB*np.array([pdv1,pdv2]),lb=0,vtype=GRB.CONTINUOUS,name='Obd')
    Sbd = M_emyo.addVars(2,d_max+1,obj=pB*np.array([pdv1,pdv2]),lb=0,vtype=GRB.CONTINUOUS,name='Sbd')
...
def get_act_mdl(d,x,At,t,T,paras,cL,p,beta):
    ...
    q0 = M_mdl.addVars(2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='q0')
    qq0 = M_mdl.addVars(2,obj=KB/2,vtype=GRB.CONTINUOUS,name='qq0')
    w0 = M_mdl.addVars(2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='w0')
    ww0 = M_mdl.addVars(2,obj=KS/2,vtype=GRB.CONTINUOUS,name='ww0')
    e0 = M_mdl.addVars(2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='e0')
    ee0 = M_mdl.addVars(2,obj=KR/2,vtype=GRB.CONTINUOUS,name='ee0')
    Ord0 = M_mdl.addVars(2,d_max+1,obj=hR*np.array([pdv1,pdv2]),lb=0,vtype=GRB.CONTINUOUS,name='Ord0')
    Srd0 = M_mdl.addVars(2,d_max+1,obj=pR*np.array([pdv1,pdv2]),lb=0,vtype=GRB.CONTINUOUS,name='Srd0')
    Obd0 = M_mdl.addVars(2,d_max+1,obj=hB*np.array([pdv1,pdv2]),lb=0,vtype=GRB.CONTINUOUS,name='Obd0')
    Sbd0 = M_mdl.addVars(2,d_max+1,obj=pB*np.array([pdv1,pdv2]),lb=0,vtype=GRB.CONTINUOUS,name='Sbd0')
    ...
    if cL>0:
        ...
        q1 = M_mdl.addVars(2,2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='q1')
        qq1 = M_mdl.addVars(2,2,obj=beta*KB/2*pAm,vtype=GRB.CONTINUOUS,name='qq1')
        w1 = M_mdl.addVars(2,2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='w1')
        ww1 = M_mdl.addVars(2,2,obj=beta*KS/2*pAm,vtype=GRB.CONTINUOUS,name='ww1')
        e1 = M_mdl.addVars(2,2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='e1')
        ...
        # M_mdl.addGenConstrMin(wl1[0],[0,b1[0]+b1[1]+q0[0]-s1-d[0]-w0[0],r1+a0[0]+e0[0]-s1-d[0]-w0[0]])
        # M_mdl.addGenConstrMin(wl1[1],[0,b2[0]+b2[1]+q0[1]-s2-d[0]-w0[1],r2+a0[1]+e0[1]-s2-d[0]-w0[1]])
        ...
        # M_mdl.addGenConstrMin(el1[0],[0,b1[0]+b1[1]+q0[0]-r1-a0[0]-e0[0],s1+d[0]+w0[0]-r1-a0[0]-e0[0]])
        # M_mdl.addGenConstrMin(el1[1],[0,b2[0]+b2[1]+q0[1]-r2-a0[1]-e0[1],s2+d[0]+w0[1]-r2-a0[1]-e0[1]])
        ...
        # M_mdl.addGenConstrMin(ql1[0],[0,s1+d[0]+w0[0]-b1[0]-b1[1]-q0[0],r1+a0[0]+e0[0]-b1[0]-b1[1]-q0[0]])
        # M_mdl.addGenConstrMin(ql1[1],[0,s2+d[0]+w0[1]-b2[0]-b2[1]-q0[1],r2+a0[1]+e0[1]-b2[0]-b2[1]-q0[1]])
        
        M_mdl.addConstrs((Ord1[0,j,k]-r1-a0[0]-e0[0]-a1[0,j]-e1[0,j]+s1+d[0]+w0[0]+k+w1[0,j]>=0 for j in range(2) for k in range(d_max+1)))
        M_mdl.addConstrs((Ord1[1,j,k]-r2-a0[1]-e0[1]-a1[1,j]-e1[1,j]+s2+d[0]+w0[1]+k+w1[1,j]>=0 for j in range(2) for k in range(d_max+1)))
        M_mdl.addConstrs((Srd1[0,j,k]+r1+a0[0]+e0[0]+a1[0,j]+e1[0,j]-s1-d[0]-w0[0]-k-w1[0,j]>=0 for j in range(2) for k in range(d_max+1)))
        M_mdl.addConstrs((Srd1[1,j,k]+r2+a0[1]+e0[1]+a1[1,j]+e1[1,j]-s2-d[0]-w0[1]-k-w1[1,j]>=0 for j in range(2) for k in range(d_max+1)))
        
        M_mdl.addConstrs((Obd1[0,j,k]-b1[0]-b1[1]-q0[0]-q1[0,j]-b1[2]+s1+d[0]+w0[0]+k+w1[0,j]>=0 for j in range(2) for k in range(d_max+1)))
        M_mdl.addConstrs((Obd1[1,j,k]-b2[0]-b2[1]-q0[1]-q1[1,j]-b2[2]+s2+d[0]+w0[1]+k+w1[1,j]>=0 for j in range(2) for k in range(d_max+1)))
        M_mdl.addConstrs((Sbd1[0,j,k]+b1[0]+b1[1]+q0[0]+q1[0,j]+b1[2]-s1-d[0]-w0[0]-k-w1[0,j]>=0 for j in range(2) for k in range(d_max+1)))
        M_mdl.addConstrs((Sbd1[1,j,k]+b2[0]+b2[1]+q0[1]+q1[1,j]+b2[2]-s2-d[0]-w0[1]-k-w1[1,j]>=0 for j in range(2) for k in range(d_max+1)))

    # period 2
    if cL>1:
        pAAm = np.array([[(1-p[0])**2,(1-p[0])*p[0],p[0]*(1-p[0]),p[0]**2],[(1-p[1])**2,(1-p[1])*p[1],p[1]*(1-p[1]),p[1]**2]])
        a2 = M_mdl.addVars(2,4,obj=beta**2*cR*pAAm,vtype=GRB.INTEGER,name='a2')
        q2 = M_mdl.addVars(2,4,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='q2')
        qq2 = M_mdl.addVars(2,4,obj=beta**2*KB/2*pAAm,vtype=GRB.CONTINUOUS,name='qq2')
        w2 = M_mdl.addVars(2,4,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='w2')
        ww2 = M_mdl.addVars(2,4,obj=beta**2*KS/2*pAAm,vtype=GRB.CONTINUOUS,name='ww2')
        e2 = M_mdl.addVars(2,4,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='e2')
        ee2 = M_mdl.addVars(2,4,obj=beta**2*KR/2*pAAm,vtype=GRB.CONTINUOUS,name='ee2')
        pAAdm1 = np.array([(1-p[0])**2*pdv1,(1-p[0])*p[0]*pdv1,p[0]*(1-p[0])*pdv1,p[0]**2*pdv1])
        pAAdm2 = np.array([(1-p[1])**2*pdv2,(1-p[1])*p[1]*pdv2,p[1]*(1-p[1])*pdv2,p[1]**2*pdv2])
        ...
        M_mdl.addConstrs((Ord2[0,j,k]-r1-a0[0]-e0[0]-a1[0,0]-e1[0,0]-a2[0,j]-e2[0,j]+s1+2*d[0]+w0[0]+w1[0,0]+k+w2[0,j]>=0 for j in [0,1] for k in range(d_max+1)))
        M_mdl.addConstrs((Ord2[0,j,k]-r1-a0[0]-e0[0]-a1[0,1]-e1[0,1]-a2[0,j]-e2[0,j]+s1+2*d[0]+w0[0]+w1[0,1]+k+w2[0,j]>=0 for j in [2,3] for k in range(d_max+1)))
        M_mdl.addConstrs((Ord2[1,j,k]-r2-a0[1]-e0[1]-a1[1,0]-e1[1,0]-a2[1,j]-e2[1,j]+s2+2*d[0]+w0[1]+w1[1,0]+k+w2[1,j]>=0 for j in [0,1] for k in range(d_max+1)))
        M_mdl.addConstrs((Ord2[1,j,k]-r2-a0[1]-e0[1]-a1[1,1]-e1[1,1]-a2[1,j]-e2[1,j]+s2+2*d[0]+w0[1]+w1[1,1]+k+w2[1,j]>=0 for j in [2,3] for k in range(d_max+1)))
        
        M_mdl.addConstrs((Srd2[0,j,k]+r1+a0[0]+e0[0]+a1[0,0]+e1[0,0]+a2[0,j]+e2[0,j]-s1-2*d[0]-w0[0]-w1[0,0]-k-w2[0,j]>=0 for j in [0,1] for k in range(d_max+1)))
        M_mdl.addConstrs((Srd2[0,j,k]+r1+a0[0]+e0[0]+a1[0,1]+e1[0,1]+a2[0,j]+e2[0,j]-s1-2*d[0]-w0[0]-w1[0,1]-k-w2[0,j]>=0 for j in [2,3] for k in range(d_max+1)))
        M_mdl.addConstrs((Srd2[1,j,k]+r2+a0[1]+e0[1]+a1[1,0]+e1[1,0]+a2[1,j]+e2[1,j]-s2-2*d[0]-w0[1]-w1[1,0]-k-w2[1,j]>=0 for j in [0,1] for k in range(d_max+1)))
        M_mdl.addConstrs((Srd2[1,j,k]+r2+a0[1]+e0[1]+a1[1,1]+e1[1,1]+a2[1,j]+e2[1,j]-s2-2*d[0]-w0[1]-w1[1,1]-k-w2[1,j]>=0 for j in [2,3] for k in range(d_max+1)))
        
        M_mdl.addConstrs((Obd2[0,j,k]-b1[0]-b1[1]-q0[0]-q1[0,0]-b1[2]-q2[0,j]-m1+s1+2*d[0]+w0[0]+w1[0,0]+k+w2[0,j]>=0 for j in [0,1] for k in range(d_max+1)))
        M_mdl.addConstrs((Obd2[0,j,k]-b1[0]-b1[1]-q0[0]-q1[0,1]-b1[2]-q2[0,j]-m1+s1+2*d[0]+w0[0]+w1[0,1]+k+w2[0,j]>=0 for j in [2,3] for k in range(d_max+1)))
        M_mdl.addConstrs((Obd2[1,j,k]-b1[0]-b1[1]-q0[1]-q1[1,0]-b1[2]-q2[1,j]-m2+s2+2*d[0]+w0[1]+w1[1,0]+k+w2[1,j]>=0 for j in [0,1] for k in range(d_max+1)))
        M_mdl.addConstrs((Obd2[1,j,k]-b1[0]-b1[1]-q0[1]-q1[1,1]-b1[2]-q2[1,j]-m2+s2+2*d[0]+w0[1]+w1[1,1]+k+w2[1,j]>=0 for j in [2,3] for k in range(d_max+1)))
        
        M_mdl.addConstrs((Sbd2[0,j,k]+b1[0]+b1[1]+q0[0]+q1[0,0]+b1[2]+q2[0,j]+m1-s1-2*d[0]-w0[0]-w1[0,0]-k-w2[0,j]>=0 for j in [0,1] for k in range(d_max+1)))
        M_mdl.addConstrs((Sbd2[0,j,k]+b1[0]+b1[1]+q0[0]+q1[0,1]+b1[2]+q2[0,j]+m1-s1-2*d[0]-w0[0]-w1[0,1]-k-w2[0,j]>=0 for j in [2,3] for k in range(d_max+1)))
        M_mdl.addConstrs((Sbd2[1,j,k]+b1[0]+b1[1]+q0[1]+q1[1,0]+b1[2]+q2[1,j]+m2-s2-2*d[0]-w0[1]-w1[1,0]-k-w2[1,j]>=0 for j in [0,1] for k in range(d_max+1)))
        M_mdl.addConstrs((Sbd2[1,j,k]+b1[0]+b1[1]+q0[1]+q1[1,1]+b1[2]+q2[1,j]+m2-s2-2*d[0]-w0[1]-w1[1,1]-k-w2[1,j]>=0 for j in [2,3] for k in range(d_max+1)))
...
# In[]:
def get_act_emdl(d,x,At,t,T,paras,cL,p,beta,Zdict):
    act = {}
    s1, b1, r1 = x[0]['s'],x[0]['b'],x[0]['r']
    s2, b2, r2 = x[1]['s'],x[1]['b'],x[1]['r']
    m1 = np.min([s1,b1[0],r1])
    ...
    for i in range(2):
        if At[i] == 0:
            Z = Zdict[i]
            a0[i] = max([0,x[i]['s']-x[i]['r']+Z[min([29,T-t])]])
    q0 = M_mdl.addVars(2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='q0')
    qq0 = M_mdl.addVars(2,obj=KB/2,vtype=GRB.CONTINUOUS,name='qq0')
    w0 = M_mdl.addVars(2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='w0')
    ww0 = M_mdl.addVars(2,obj=KS/2,vtype=GRB.CONTINUOUS,name='ww0')
    ...
    if cL>0:
        pAm = np.array([[1-p[0],p[0]],[1-p[1],p[1]]])
        Z1 = Zdict[0][min(29,T-t-1)]
        Z2 = Zdict[1][min(29,T-t-1)]
        a1 = M_mdl.addVars(2,2,obj=beta*cR*pAm,lb=0,vtype=GRB.INTEGER,name='a1')
        q1 = M_mdl.addVars(2,2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='q1')
        qq1 = M_mdl.addVars(2,2,obj=beta*KB/2*pAm,vtype=GRB.CONTINUOUS,name='qq1')
        w1 = M_mdl.addVars(2,2,lb=-float('inf'),obj=0,vtype=GRB.INTEGER,name='w1')
        ...
        M_mdl.addConstrs((a1[i,1]==0 for i in range(2)))
        M_mdl.addConstr(a1[0,0]-s1-d[0]-w0[0]+r1+a0[0]+e0[0]-Z1>=0)
        M_mdl.addConstr(a1[1,0]-s2-d[1]-w0[1]+r2+a0[1]+e0[1]-Z2>=0)
        M_mdl.addConstrs((qq1[i,j]-q1[i,j]>=0 for i in range(2) for j in range(2)))
        M_mdl.addConstrs((qq1[i,j]+q1[i,j]>=0 for i in range(2) for j in range(2)))
        M_mdl.addConstrs((ww1[i,j]-w1[i,j]>=0 for i in range(2) for j in range(2)))
        M_mdl.addConstrs((ww1[i,j]+w1[i,j]>=0 for i in range(2) for j in range(2)))
    ...
    if cL>1:
        ...
        M_mdl.addConstrs((a2[i,j]==0 for i in range(2) for j in [1,3]))
        M_mdl.addConstrs((a2[0,j]-s1-2*d[0]-w0[0]-w1[0,k]+r1+a0[0]+a1[0,k]+Z1>=0 for j in [0,2] for k in range(2)))
        M_mdl.addConstrs((a2[1,j]-s2-2*d[1]-w0[1]-w1[1,k]+r2+a0[1]+a1[1,k]+Z2>=0 for j in [0,2] for k in range(2)))
        M_mdl.addConstrs((qq2[i,j]-q2[i,j]>=0 for i in range(2) for j in range(4)))
        M_mdl.addConstrs((qq2[i,j]+q2[i,j]>=0 for i in range(2) for j in range(4)))
        M_mdl.addConstrs((ww2[i,j]-w2[i,j]>=0 for i in range(2) for j in range(4)))
        M_mdl.addConstrs((ww2[i,j]+w2[i,j]>=0 for i in range(2) for j in range(4)))
