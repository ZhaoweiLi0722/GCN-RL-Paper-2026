"""Prepare zero-trajectory source states for the Stage C3 DDPG pairs."""
...
SEEDS = (50, 51, 52)
RUN_NAME = (
    "patient_indexed_specimen_routing_ddpg_structured_exploration_preonline"
)
...
def prepare_preonline_states(
    ...
    *,
    ...
    if int(base["online_episodes"]) != 100:
        raise ValueError("Stage C3 source control must lock 100 episodes")
    structured = base["config_overrides"]["residual_action"][
        "structured_exploration"
    ]
    if bool(structured.get("enabled", True)):
        raise ValueError(
            "Stage C3 source must use the disabled structured-exploration arm"
        )
    ...
    for algorithm in ALGORITHMS:
        for seed in SEEDS:
            ...
            config["experimental_role"] = (
                "zero-trajectory source state shared by unchanged and "
                "structured-exploration DDPG"
            )
    ...
    if latest is None:
        raise RuntimeError("No Stage C3 pre-online state was prepared")
    return latest
