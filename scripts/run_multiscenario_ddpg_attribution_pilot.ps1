...
$RunRoot = "results\multiscenario_gcn_flat_ddpg_attribution_cuda_pilot"
$FinalOutput = (
    "results\multiscenario_gcn_flat_ddpg_attribution_cuda_pilot_eval"
)
$FrozenOutput = (
    "results\" +
    "multiscenario_gcn_flat_ddpg_attribution_cuda_pilot_pretrain_eval"
)
...
$LogPath = Join-Path $TransferRoot "pilot_$Timestamp.txt"
$ComparisonPath = Join-Path $TransferRoot (
    "checkpoint_attribution_$Timestamp.json"
)

$TrainingConfig = (
    "experiments\configs\" +
    "multiscenario_gcn_flat_ddpg_attribution_cuda_pilot.json"
)
$FinalEvaluationConfig = (
    "experiments\configs\" +
    "multiscenario_gcn_flat_ddpg_attribution_cuda_pilot_eval.json"
)
$FrozenEvaluationConfig = (
    "experiments\configs\" +
    "multiscenario_gcn_flat_ddpg_attribution_cuda_pilot_pretrain_eval.json"
)
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Multi-scenario DDPG training failed."
    }
...
$ZipPath = Join-Path $TransferRoot (
    "multiscenario_ddpg_attribution_pilot_$Timestamp.zip"
)
$ArchiveInputs = @(
    $RunRoot,
    $FinalOutput,
...
Write-Host ""
Write-Host "Multi-scenario DDPG attribution pilot completed."
