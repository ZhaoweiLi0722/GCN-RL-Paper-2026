"""Matched TD3 for the complete flat AFR residual policy surface."""

from __future__ import annotations

from typing import Any

from src.baselines.flat_ddpg import FlatDDPGAgent
from src.rl.matched_residual_td3 import MatchedResidualTD3Mixin


class ConservativeFlatResidualTD3Agent(
    MatchedResidualTD3Mixin,
    FlatDDPGAgent,
):
    """TD3 critic backbone with the routing-primary flat-DDPG contract."""

    algorithm = "flat_residual_mdl2_network_td3_bc"

    def __init__(
        self,
        state_dim: int,
        action_dim: int,
        config: dict[str, Any],
    ):
        super().__init__(state_dim, action_dim, config)
        self._initialize_matched_td3(config)

    def _td3_state_views(self, raw_states):
        critic_states = (
            raw_states
            if self.temporal_demand_encoder_enabled
            else self.observation_scaler.normalize_tensor(raw_states)
        )
        actor_inputs = self._actor_input_tensor(
            raw_states,
            normalized_states=critic_states,
        )
        return critic_states, actor_inputs
