"""Detached launcher for the locked Stage C2 DDPG campaign."""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=(
            "experiments/configs/"
            "patient_indexed_specimen_routing_ddpg_persistent_shift_"
            "execution.json"
        ),
    )
    ...
    command = [
        sys.executable,
        "-m",
        "evaluation.run_patient_indexed_specimen_routing_ddpg_persistent_shift",
        "--expected-commit",
    ...
    print(f"Stage C2 detached PID: {process.pid}", flush=True)
